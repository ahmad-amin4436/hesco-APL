...
$(document).ready(function () {
    ...
    const selectFilters = [
        { selector: '#IMSIFilter', type: 'imsi' },
        { selector: '#SimNumberFilter', type: 'simnumber' }
    ];
    ...
    var table = $('#imsiTable').DataTable({
        ...
        ajax: {
            ...
            data: function (d) {
                return {
                    draw: d.draw,
                    fimsi: $('#IMSIFilter').val(),
                    fsimNumber: $('#SimNumberFilter').val(),
                    foperator: $('#OperatorFilter').val() ? $('#OperatorFilter').val().join(',') : null,
                    fproject: $('#ProjectFilter').val() ? $('#ProjectFilter').val().join(',') : null,
                    fchangeProject: $('#ChangeProjectFilter').val(),
                    fstatus: $('#StatusFilter').val(),
                    fcreatedBy: $('#CreatedByFilter').val(),
                    fupdatedBy: $('#UpdatedByFilter').val(),
                    fcreatedAt: $('#CreatedAtFilter').val(),
                    fupdatedAt: $('#UpdatedAtFilter').val(),
                    fmapDateTime: $('#MapDateTimeFilter').val(),
                    fchangeProjectDate: $('#ChangeProjectDateFilter').val(),
                    fissuedTo: $('#IssuedToFilter').val(),
                    fsimStatus: $('#SimStatusFilter').val(),
        ...
        rowGroup: {
            ...
            startRender: function (rows, group) {
                ...
                if (hasFilter) {
                    ...
                    return $('<tr/>')
                        .append('<td colspan="16" class="group-row" style="cursor:pointer; font-weight:bold; background:#f1f3f5;">' +
        ...
        columns: [
            ...
            {
                data: 'issued_to_username',
                render: function (data, type, row) {
                    return data || row.issued_to || '-';
                }
            },
            {
            ...
            },
            {
                ...
                orderable: false,
                render: function (data) {
                    return renderIMSIActionButtons(data, table);
                }
            }
        ],
        pageLength: 50,
        ...
        drawCallback: function (settings) {
            ...
            $('#imsiTable tbody').off('click', '.toggle-btn').on('click', '.toggle-btn', function () {
    ...
    function exportToExcel(exportType) {
        ...
        window.location.href = imsiUrls.exportExcel + '?' + $.param(params);
    }
    ...
    function exportToPDF(exportType) {
        ...
        window.location.href = imsiUrls.exportPDF + '?' + $.param(params);
    }
    ...
    // Render action buttons for IMSI
    function renderIMSIActionButtons(data, table) {
        let id = data.id;
        let imsi = data.imsi;

        return `
            <div class="btn-group btn-group-sm" role="group" aria-label="IMSI Actions">
                <a href="javascript:void(0);" title="View" class="btn btn-outline-info view-imsi-btn" data-id="${id}">
                    <i class="fas fa-eye"></i>
                </a>
                <button class="btn btn-outline-danger delete-imsi-btn" data-id="${id}" title="Delete">
    ...
    // AJAX Event Handlers for IMSI Action Buttons
    $(document).on('click', '.view-imsi-btn', function (e) {
        e.preventDefault();
        let id = $(this).data('id');
        viewIMSIDetails(id);
    });

    $(document).on('click', '.delete-imsi-btn', function (e) {
        e.preventDefault();
        let id = $(this).data('id');
        deleteIMSI(id);
    });

    // AJAX function to view IMSI details
    function viewIMSIDetails(id) {
        $.ajax({
            url: imsiUrls.getIMSIDetails,
            type: 'GET',
            data: { id: id },
            beforeSend: function () {
                showLoading('Loading IMSI details...');
            },
            success: function (response) {
                hideLoading();
                if (response.success) {
                    showIMSIModal(response.data, 'View IMSI Details');
                } else {
                    showNotification(response.message || 'Error loading IMSI details', 'error');
                }
            },
            error: function (xhr, status, error) {
                hideLoading();
                showNotification('Error loading IMSI details: ' + error, 'error');
            }
        });
    }

    // AJAX function to delete IMSI
    function deleteIMSI(id) {
        if (!confirm('Are you sure you want to delete this IMSI record?')) {
            return;
        }

        $.ajax({
            url: imsiUrls.deleteIMSI,
            type: 'POST',
            data: { id: id },
            beforeSend: function () {
                showLoading('Deleting IMSI...');
            },
            success: function (response) {
                hideLoading();
                if (response.success) {
                    showNotification('IMSI deleted successfully!', 'success');
                    $('#imsiTable').DataTable().ajax.reload(null, false);
                } else {
            ...
            error: function (xhr, status, error) {
                hideLoading();
                showNotification('Error deleting IMSI: ' + error, 'error');
            }
        });
    }

    // Show IMSI details in modal
    function showIMSIModal(data, title) {
        // Format dates
        ...
        // Create modal HTML for IMSI
        const modalHtml = `
            <div class="modal fade" id="imsiModal" tabindex="-1" role="dialog">
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label><strong>IMSI:</strong></label>
                                        <p class="form-control-plaintext">${data.imsi || '-'}</p>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label><strong>SIM Number:</strong></label>
                                        <p class="form-control-plaintext">${data.sim_number || '-'}</p>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <p class="form-control-plaintext">${data.operator_name || '-'}</p>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label><strong>Plan Name:</strong></label>
                                        <p class="form-control-plaintext">${data.plan_name || '-'}</p>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label><strong>SIM Status:</strong></label>
                                        <p class="form-control-plaintext">${data.sim_status || '-'}</p>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <p class="form-control-plaintext">${data.issued_to_username || '-'}</p>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <p class="form-control-plaintext">${data.monthly_bill || '-'}</p>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label><strong>Credit Limit:</strong></label>
                                        <p class="form-control-plaintext">${data.credit_limit || '-'}</p>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <p class="form-control-plaintext">${data.meter_type || '-'}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    function initIMSISearch_DDL(selector, type, isMultiple = false) {
        $.ajax({
            url: imsiUrls.loadIMSISearchDDL,
            type: "GET",
            dataType: "json",
    ...
    // List of filters
    const IMSIFilters = [
        { select: '#OperatorFilter', type: 'operator', isMultiple: true },
        { select: '#PlanNameFilter', type: 'planname', isMultiple: false },
        ...
        { select: '#SimStatusFilter', type: 'simstatus', isMultiple: false },
        { select: '#IssuedToFilter', type: 'issuedto', isMultiple: false },
        { select: '#CreatedByFilter', type: 'createdby', isMultiple: false },
        { select: '#UpdatedByFilter', type: 'updatedby', isMultiple: false }
    ];

    // Initialize all filters
    IMSIFilters.forEach(f => initIMSISearch_DDL(f.select, f.type, f.isMultiple));
});